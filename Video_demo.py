...
if __name__ == '__main__':
    ...
    model_checkpoint = torch.load('saving_CASIA_ckpt_0526/Iter_00414000_model.ckpt', map_location=lambda storage, loc: storage)
    ...
    if args.update:
        ...
        # targets: number of candidate x 256

    cap = cv2.VideoCapture('video/TheBigBangTheory.mp4')
    ...
    while True:
        ...
        if isSuccess:
            try:
                ...
                source_embs = torch.cat(embs)  # number of detected faces x 256
                diff = source_embs.unsqueeze(-1) - targets.transpose(1, 0).unsqueeze(0) # i.e. 3 x 256 x 1 - 1 x 256 x 2 = 3 x 256 x 2
