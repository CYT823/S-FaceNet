...
if __name__ == '__main__':
    ...
    # embeding size is 512 to 128 (feature vector)
    mobile_face_net_model = MobileFaceNet(256) 
    if torch.cuda.is_available():
        model_checkpoint = torch.load('saving_CASIA_ckpt_0526/Iter_{:0>8d}_model.ckpt'.format(args.iterations))
    else:
        model_checkpoint = torch.load('saving_CASIA_ckpt_0526/Iter_{:0>8d}_model.ckpt'.format(args.iterations), map_location="cpu")
    mobile_face_net_model.load_state_dict(model_checkpoint['net_state_dict'])
    mobile_face_net_model.to(device)
    print('MobileFaceNet face detection model generated.')
    
    mobile_face_net_model.eval()

    if args.update:
        targets, names = prepare_facebank(mobile_face_net_model, path='facebank', tta=args.tta)
            ...
            try:
                ...
                for img in faces:
                    if args.tta:
                        ...
                        emb = mobile_face_net_model(test_transform(img).to(device).unsqueeze(0))
                        emb_mirror = mobile_face_net_model(test_transform(mirror).to(device).unsqueeze(0))
                        embs.append(l2_norm(emb + emb_mirror))
                    else:
                        embs.append(mobile_face_net_model(test_transform(img).to(device).unsqueeze(0)))
